    ...
    items: [
      {
        ...
        answer: "這是教育部推動的計畫，目標達成「班班有網路，生生用平板」。在此方案下的 iPad **必須** 受 MDM (行動裝置管理系統) 納管。這跟一般企業或個人的設備主要差異在於：「載具不屬於學生個人，而是學校資產」，因此會有較嚴格的限制（如無法自行下載 App、有瀏覽紀錄保存等），目的是確保載具專注於學習用途。",
