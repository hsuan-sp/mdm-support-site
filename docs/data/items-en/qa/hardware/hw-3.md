...
1.  **VLAN Consistency**: This is the most common campus issue. If a teacher’s iPad is on the `Staff-WiFi` network but the Apple TV is on the `Student-IoT` network, and those networks are isolated, AirPlay packets cannot find their destination.
